...
while(l!=""):
	...
	if elements[0]=="POSITION":
		...
		points.append([float(elements[0][1:]), float(elements[1][1:]), float(elements[2][3:]), float(elements[3][1:]), float(elements[4][1:])])	
...
for i in range(len(points)):
	outfile.write(str(points[i][0])+";"+str(points[i][1])+";"+str(points[i][2])+";"+str(points[i][3])+";"+str(points[i][4])+"\n")
outfile.close()
